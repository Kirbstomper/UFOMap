...
app.get('/addSearch',function(req,res){
  ...
  connection.query('SELECT * FROM mapped_clean WHERE '+euc+' < .72 order by '+ euc ,(err,results,fields)=>{
    if(err)
...
app.get('/airport',function(req,res){
  ...
  connection.query('SELECT * FROM airports WHERE '+euc+' < .72 order by '+ euc ,(err,results,fields)=>{
    if(err)
      return res.status(400).send({error:'Database error',message:err});
    res.status(200).send(results);
  })
