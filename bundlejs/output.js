...
function drawSearchMap(addrs) {
    ...
    request.get('https://maps.googleapis.com/maps/api/geocode/json?address=' + addrs + '&key=AIzaSyDBU4eMBD02OnA8bu0iIn4HZFE__DEW3gE').set('Accept', 'application/json').end((err, res) => {
        if (res) {
            ...
            request.get('/addSearch?lat=' + lat + '&long=' + long).set('Accept', 'application/json').end((err, res) => {
                if (res) {
                    console.log(res)
                    var sap = 0;
                    var total = 0;
                    ...
                    for (i = 0; i < res.body.length; i++) {
                        ...
                        sap++;
                        total++;
                        var x_string = '<div id ="content">' + '<h1 id="firstHeading" class = "firstHeading">Median Income: ' + x.income + '</h1>' + '<div id="bodyContent">' + '<p><b> Shape: <b>' + x.shape + '<p>' + '<p><b> Sighting Time: <b>' + x.datetime + '<p>' + '<p><b> Location : <b>' + x.city + ", " + x.state + '<p>' + '<p><b> Duration(seconds): <b>' + x.duration + '<p>' + '<p><b> Sightings at this Location: <b>' + sap + '<p>' + '</div>' + '</div>';
                        data.addRows([
                            [x.lat, x.long, x_string, "ufo"]
                        ]);
                    }
                    //Get airport points
                    request.get('/airport?lat=' + lat + '&long=' + long).set('Accept', 'application/json').end((err, res) => {
                        if (res) {
                            for (i = 0; i < res.body.length; i++) {
                                var a = res.body[i];
                                data.addRows([
                                    [a.Latitude, a.Longitude * (-1), a.locationID, 'airport']
                                ]);
                            }
                            map.draw(data, {
                                showTooltip: true,
...
function drawMapAdress() {
    ...
    var map = new google.visualization.Map(document.getElementById('map_div'));
    request.get('/testSearch').set('Accept', 'application/json').end((err, res) => {
        if (res) {
            console.log(res)
            for (i = 0; i < res.body.length; i++) {
                var x = res.body[i];
